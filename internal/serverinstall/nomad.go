...
func waypointNomadJob(c nomadConfig, rawRunFlags string) *api.Job {
	...
	task.Config = map[string]interface{}{
		...
		"args":           []string{"server", "run", "-accept-tos", "-vvv", "-db=/alloc/data/data.db", fmt.Sprintf("-listen-grpc=0.0.0.0:%s", defaultGrpcPort), fmt.Sprintf("-listen-http=0.0.0.0:%s", defaultHttpPort), rawRunFlags},
