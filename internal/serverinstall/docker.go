...
func (i *DockerInstaller) Install(
	...
	cfg := container.Config{
		...
		Cmd:          []string{"server", "run", "-accept-tos", "-vvv", "-db=/data/data.db", fmt.Sprintf("-listen-grpc=0.0.0.0:%s", grpcPort), fmt.Sprintf("-listen-http=0.0.0.0:%s", httpPort), opts.ServerRunFlags},
	}
